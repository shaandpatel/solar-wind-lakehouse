...
1. Ingestion (01_ingest_bronze.py)
Source: Historical solar wind telemetry (historicsw.csv) loaded from Unity Catalog Volumes.

Schema Enforcement: Enforces strict typing (TimestampType, DoubleType) via PySpark StructType during read.

Target Table: workspace.default.sw_bronze.

2. Validation & Quarantine (02_clean_silver_quarantine.py)
Data Quality Gate: Checks for missing primary keys (time_tag) and invalid telemetry values (e.g., negative wind speed, null values).

Quarantine Pattern: Routes failed records to sw_quarantine appended with specific quarantine_reason tags.

Merge: Clean records are upserted into sw_silver using Delta MERGE INTO matching on time_tag.

3. Feature Engineering & ML Training (03_gold_mlflow_train.py)
Time-Series Features: Computes 16 windowed metrics partitioned by month (DATE_TRUNC('month', time_tag)):
...
Target Table: Writes evaluation output to sw_gold_training_eval.

4. Batch Inference & Optimization (04_batch_inference_zorder.py)
Dynamic Fetching: Programmatically fetches the latest model run from MLflow using experiment paths.

Performance Tuning: Executes OPTIMIZE sw_gold_anomalies ZORDER BY (time_tag) to co-locate row groups for fast multi-dimensional querying.

Time Travel Audit: Verifies record consistency across Delta table versions via DESCRIBE HISTORY.
